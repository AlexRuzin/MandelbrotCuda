...
#include "../FractalCore/types.h"
#include "../FractalCore/debug.h"
#include "../FractalCore/sdl_render.h"
...
void loopTimer::cuda_render_thread(loopTimer *controller)
{
	...
	const config::iniContents::CUDA &config = controller->cfg->iniData.cuda;
	...
	while (controller->threadStateCuda == THREAD_STATE_RUNNING) {
		Sleep(config.CONTROLLER_LOOP_WAIT);
		...
		case SET_ZOOM_REVERSE:
			newSCALEA = config.ZOOM_ALPHA * kernel->getScaleA() * std::exp(-config.ZOOM_BETA * lastSCALEA);
			if (newSCALEA >= config.MAX_DELTA_SCALE_A) {
				newSCALEA = 0.0;
			}
			break;
...
{
	assert(inMouseX <= cfg->iniData.cuda.FRAME_BUFFER_HEIGHT && 
		inMouseY <= cfg->iniData.cuda.FRAME_BUFFER_LENGTH);
	...
	if (inMouseX > 0.0f) {
		mouseX = (double)inMouseX * (2.0 / (double)cfg->iniData.cuda.FRAME_BUFFER_LENGTH) - 1.0;
	}

	if (inMouseY > 0.0f) {
		mouseY = (double)inMouseY * (2.0 / (double)cfg->iniData.cuda.FRAME_BUFFER_HEIGHT) - 1.0;
	}
